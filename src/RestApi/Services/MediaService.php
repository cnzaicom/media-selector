...
class MediaService
{
    ...
    public function upload($userId, UploadedFile $file, $type, $move)
    {
        ...
        $result = Media::query()->where('md5', '=', $md5)->select(['user_id', 'path',
            'file_name', 'size', 'type', 'file_ext', 'disk', 'bucket', 'meta', 'id'])->first();

        if ($result) {
            $result->name = $result->file_name;
            $result->media_type = $result->type;
            $result->width = $result->meta['width'];
            $result->height = $result->meta['height'];
            $result->url = Storage::disk('admin')->url($result->path);
            return $result;
        }
